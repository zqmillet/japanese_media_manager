...
def test_media_finder(directory, file_paths):
    ...
    assert list(media_finder) == list(map(pathlib.Path, ['for_test/star-325.mp4', 'for_test/松本菜奈実/SSNI-306/ssni-306.mkv']))
    assert list(media_finder) == list(map(pathlib.Path, ['for_test/star-325.mp4', 'for_test/松本菜奈実/SSNI-306/ssni-306.mkv']))

    media_finder = MediaFinder(directories=[directory], recursively=False, extensions=['.mkv', '.mp4'])
    assert list(media_finder) == list(map(pathlib.Path, ['for_test/star-325.mp4']))
