...
def test_media_finder(directory, file_paths):
    ...
    assert [info.file_path for info in media_finder] == list(map(pathlib.Path, ['for_test/star-325.mp4', 'for_test/松本菜奈実/SSNI-306/ssni-306.mkv']))
    assert not list(media_finder)[0].has_chinese_subtitle
    assert not list(media_finder)[1].has_chinese_subtitle
    assert list(media_finder)[0].number == 'STAR-325'
    assert list(media_finder)[1].number == 'SSNI-306'
